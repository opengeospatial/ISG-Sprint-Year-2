...
.Feature information in the RMTexture from Aden, Yemen CDB stored as TIFF. CDB was provided by Presagis._
...
[NOTE]
====
_There is a sudden switch from XML to JSON here with no expositopry transition. A little more needs to be done to separate these two (XML vs JSON) sections. Perhaps a new report section for each of these._
====
...
[NOTE]
====
_It would be very useful to indicate that the enums are refenced by *"value"*, and the value of *value* is arbitrary within the UINT8 data type._
====
...
._CesiumJS visualization of the per-texel metadata from Aden, Yemen CDB. The metadata was overlaid on the satellite imagery of the terrain._
...
._CesiumJS visualization of the per-texel metadata from Aden, Yemen CDB at a low-level of detail. Unlike <<Cesium_RMTexture_TerrainOverlay>> there were no overlays involved._
...
._CesiumJS visualization of the per-texel metadata from Aden, Yemen CDB at a high-level of detail taken near the center of <<Cesium_RMTexture_MetadataLoLOD>>._
...
[#Cesium_RMTexture_TIFF,reftext='{figure-caption} {counter:figure-num}']
._The per-vertex metadata (as opposed to the per-texel metadata shown in figures <<Cesium_RMTexture_TerrainOverlay>> through <<Cesium_RMTexture_MetadataHiLOD>>) from CDB of Aden, Yemen CDB and visualized in CesiumJS_. The verticies were classified and color coded according to their metadata.
...
.Using the CesiumJS renderer meshes from GeoTypical tree models are stored once and rendered from CDB of Aden, Yemen._
...
._An image showing the folder Structure with *3DTILES_implicit_tiling* extension._
